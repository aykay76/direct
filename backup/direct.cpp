...
HRESULT Process()
{
	...
	if (GetTickCount() - g_dwLastWave >= 1000) {
		if (fCreateWave) {
			Wave *wave = new Wave();
			wave->Create(5);
...
*/
	...
	hr = g_lpDDSBack->BltFast(100, 100, g_lpDDSAlien, &rc, DDBLTFAST_SRCCOLORKEY);
	if (hr != DD_OK) {
		if (hr == DDERR_SURFACELOST) {
			// The surfaces were lost so restore them 
			OutputDebugString("Restoring surface\n");
			g_lpDDSAlien->Restore();
			HBITMAP hbm = (HBITMAP)LoadImage(hInst, MAKEINTRESOURCE(IDB_ALIEN), IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION);
			if (FAILED(hr = DDUtil_CopyBitmap(g_lpDDSAlien, hbm, 0, 0, 20, 20))) {
				char szMessage[256];
				sprintf(szMessage, "Failed to copy alien bitmap: (%08x)", hr);
				MessageBox(g_hWnd, szMessage, "Fatal error", MB_OK | MB_ICONHAND);
		...
		char szMessage[128];
		sprintf(szMessage, "Failed to blit alien: %08x\n", hr);
